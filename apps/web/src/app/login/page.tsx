  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      if (user.status === "rejected") {
        setError(
          "Your registration has been declined. Please contact us for more information.",
        );
        setIsSubmitting(false);
        return;
      }
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-0 left-0 z-0 pointer-events-none select-none opacity-[0.15] grayscale">
          <CoffeeBranchImage className="w-[500px] h-[500px] translate-y-1/4 -translate-x-1/4 rotate-12 filter invert" />
        </div>
      </div>
